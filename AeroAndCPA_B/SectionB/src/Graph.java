...
public class Graph implements GraphInterface{
	...
	private void setIncomingDegreeRecursively(GraphNode node) {
		...
		node.getOutgoingEdges().forEach(
				edge -> setIncomingDegreeRecursively(edge.getEndPoint())
		);
	}
	...
	private QueueInterface<GraphNode> topologicalSort( ) {
		...
		while (!temporary.isEmpty()) {
			GraphNode parent = temporary.dequeue();
			result.enqueue(parent);
			parent.getOutgoingEdges().forEach(
					edge -> {
						GraphNode child = edge.getEndPoint();
						child.setDegree(child.getDegree() - 1);
						if (child.getDegree() == 0) {
							temporary.enqueue(child);
						}
					}
			);
		}
